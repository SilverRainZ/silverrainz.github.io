...
把一个集合分成两个，使其 sum 分别相等。假设原集合 sum 为 |Sa| ，从集合中选出一个子集，使其 sum 刚好为 |Sa|/2 ，剩下的元素组成的子集的 sum 自然也为 |Sa|/2。因此问题可以转化为：

一个 :ref:`01-pack` 问题，背包容量为 |Sa|/2，要求恰好装满，填充物的 cost 是数字的值，value 统一设置为 1，因为只需要证有解。
